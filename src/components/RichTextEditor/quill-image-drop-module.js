...
export class ImageDrop {
  ...
  insert(dataUrl) {
    ...
    this.quill.insertEmbed(index, "image", dataUrl, "user");
    //粘贴时会出现无效图片，需要删除
    const quill_e = this.quill.getContents(); //获取编辑器中的内容
    if (quill_e && quill_e.ops) {
      if (quill_e.ops.length > 0) {
        //查找空白图片并删除
        const blank_img_index = quill_e.ops.findIndex(
          (item) => item.insert && item.insert.image == "//:0"
        );
        if (blank_img_index > -1) {
          quill_e.ops.splice(blank_img_index, 1);
        }
        //去除重复图片
        const more_img = quill_e.ops.filter(
          (item) => item.insert && item.insert.image == dataUrl
        );
        if (more_img.length >= 2) {
          const more_index = quill_e.ops.findIndex(
            (item) => item.insert && item.insert.image == dataUrl
          );
          quill_e.ops.splice(more_index, 1);
        }
      }
    }
    console.log(quill_e);
    this.quill.setContents(quill_e);
    // 调整光标到最后
